...
{	
	...
	public function show()
	{
        ...
        if ($attribute_id==0||$attribute_id=='') {
           $arr=Db::query("select * from attrdetails");
        }else{
            $arr=Db::query("select * from attrdetails where attr_id='$attribute_id'");
        }
	...
	public function addAction()
	{
        ...
        // echo "attrcate_id";
        // die;
        $arr=Db::query("select * from attrdetails where name='$name' and attr_id='$attribute_id'");

        if (empty($arr)) {
            $arr=Db::query("insert into `attrdetails` (`name`,`attr_id`) values ('$name','$attribute_id')");
